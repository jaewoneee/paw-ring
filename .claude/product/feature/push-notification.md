...
### 알림 스케줄링
- [x] 단건 스케줄 알림 등록 (로컬 알림)
- [x] 반복 스케줄 알림 등록 (향후 7일치 일괄 등록, 앱 실행 시 보충)
- [x] 스케줄 수정/삭제 시 알림 업데이트/취소
- [x] 알림 시간 옵션 (스위치 토글 ON/OFF)
...
### 알림 ON/OFF
- [x] 스케줄 단위 알림 ON/OFF
- [x] 캘린더(반려동물) 단위 알림 ON/OFF
- [x] 전체 알림 ON/OFF (설정 화면)

### 알림 수신
- [x] 포그라운드 알림 표시 (앱 사용 중)
- [x] 백그라운드 알림 표시
- [x] 알림 탭 시 해당 스케줄 상세로 딥링크
...
├── schedule.ts                   # 스케줄 CRUD 시 알림 자동 등록/취소 연동 ✅

hooks/
├── useNotification.ts            # 알림 권한 상태, 딥링크, 알림 보충 등록 ✅

utils/
├── notificationScheduler.ts      # 로컬 알림 스케줄 계산/등록/취소 ✅
```
