...
- **원격 알림**: Firebase Cloud Messaging (공유 대상자 알림)
...
### 공유 권한
- [ ] 열람 (viewer): 스케줄 조회, 완료 체크
- [ ] 편집 (editor): 스케줄 CRUD + 완료 체크
- [ ] 소유자 (owner): 모든 권한 + 멤버 관리 + 삭제
...
services/
├── sharing.ts                        # 공유 관련 CRUD (Firestore)
├── invite.ts                         # 초대 생성/수락/거절
...
### Firestore: `calendarShares` 컬렉션
```typescript
// calendarShares/{petId}_{userId}
// 복합 키: 반려동물 ID + 공유 대상 유저 ID
interface CalendarShare {
  petId: string;                      // 반려동물 ID
  petName: string;                    // 반려동물 이름 (조회 편의)
  ownerId: string;                    // 캘린더 소유자 UID
  ownerNickname: string;              // 소유자 닉네임
  sharedUserId: string;               // 공유 대상 유저 UID
  sharedUserNickname: string;         // 공유 대상 닉네임
  role: 'viewer' | 'editor';         // 권한
  status: 'pending' | 'accepted';    // 초대 상태
  createdAt: Timestamp;
  updatedAt: Timestamp;
}
```

### Firestore: `invites` 컬렉션 (링크 초대용)
```typescript
// invites/{inviteId}
interface Invite {
  id: string;                         // 초대 ID (딥링크에 사용)
  petId: string;
  petName: string;
  ownerId: string;
  ownerNickname: string;
  role: 'viewer' | 'editor';
  expiresAt: Timestamp;               // 만료 시간 (예: 7일)
  usedBy: string | null;              // 사용한 유저 UID (1회용일 경우)
