...
1. `.claude/product/` 디렉토리에서 `$ARGUMENTS`와 관련된 기획 문서를 먼저 찾아 읽는다.
   - 기획 문서의 요구사항, 권한 정의, 시나리오를 테스트 케이스 설계의 기준으로 삼는다.
2. `$ARGUMENTS`에 해당하는 소스 코드를 찾아서 읽는다.
3. 기획 문서 요구사항과 소스 코드를 대조하여 입력/출력, 분기 조건, 경계값을 분석한다.
4. `__tests__/` 디렉토리에 테스트 파일을 작성한다.
   - 파일명: `__tests__/{기능명}.test.ts`
   - 기존 테스트 파일이 있으면 테스트 케이스를 추가한다.
5. `npx jest {테스트파일}` 으로 테스트를 실행한다.
6. 실패하는 테스트가 있으면:
   - 테스트가 잘못된 경우 → 테스트를 수정
